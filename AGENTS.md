...
| `scripts/` | `bootstrap-vps.sh` (host setup), `deploy.sh` (idempotent redeploy), `check-doc-drift.sh`, `docker-builder-gc.sh` (Docker BuildKit cache cap — writes `/etc/docker/daemon.json`, needs an operator restart to apply, see `docs/runbook.md` "Applying the Docker builder cache cap"), `tmp-scratch-policy.sh` (masks the tmpfs `/tmp` mount unit + tightens its `systemd-tmpfiles` age so build/session scratch lives on disk and self-retires — operator live-cutover in `docs/runbook.md` "Moving /tmp off RAM"), memory-audit cron wrapper (audit logic lives in the vault `bin/`), `quota-share/` (read-only report: shares quota-axi's account % across consumers by logged Claude Code session usage — see `scripts/quota-share/README.md`), sync tooling | The VPS |
