...
#### Vault search for every agent (`memory.search.extraPaths`, in the platform file since 2026-09-23)
...
without a second indexer. The index this key builds is a derived layer:
memory-core writes into each agent's own sqlite store under its workspace,
never under the vault, so the vault stays read-only from the gateway's side
(decision record: vault `system/proposals.md`,
