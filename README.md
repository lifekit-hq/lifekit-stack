...
| `prometheus` | `prom/prometheus:v2.54.1` | Box-level metrics: scrapes finance-sentry's API, devclaw-mcp's `/metrics` (the dead-man signal) and the gateway's `/api/diagnostics/prometheus` (bearer token from the `openclaw_gateway_token` compose secret). 30d / 5GB retention. Loopback `:9090`. |
