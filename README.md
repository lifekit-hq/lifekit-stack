...
- **OpenClaw layer** - the `diagnostics-prometheus` plugin is baked into `lifekit-openclaw` at `/opt/openclaw-plugins/diagnostics-prometheus` (same version as the base image's core) and loaded through `plugins.load.paths` in the host `openclaw.json`. The `openclaw` rule group's *scheduled or background run failed* fires on the first failed agent run in 15 minutes: any outcome but `completed` for cron runs (a timeout reads `aborted`), `error` for every other trigger. The same rule fires with `trigger=exporter`, `outcome=series_dropped` when the exporter hits its 2048-series cap and starts refusing new series (new failure series may then be missing until the gateway restarts). OpenClaw 2026.9.4 exports no cron- or task-ledger series, so command-kind cron jobs and `lost` tasks are not covered: `openclaw tasks audit` is still the check for those.
