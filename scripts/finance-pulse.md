...
1. get_pending_companion_events(includeHeldForDigest=true): anything held for the digest or older than 6h -> one grouped message, then record_event_verdict for every event judged this pass with a short plain-text verdict of what happened and what it means, not the digest repeated (notified=true if your message covered that event, notified=false if you judged it immaterial; a recorded=false result means the event cannot carry a verdict (not yours, missing, not captured from an alert, or a blank verdict), do not retry or rephrase the call), then acknowledge_companion_events for every event judged this pass.
