...
1. get_pending_companion_events(includeHeldForDigest=true): anything held for the digest or older than 6h -> one grouped message, then record_event_verdict for every event the digest covered (notified=true) and for any event judged immaterial in this pass (notified=false); a recorded=false result means the event cannot carry a verdict (not yours, missing, not captured from an alert, or a blank verdict), do not retry or rephrase the call, still acknowledge the event; then acknowledge_companion_events for what you reported.
